...
5. **Client lifecycle** — `src/BKTClient.ts`. `initializeBKTClientInternal` creates the
   singleton (`internal/instance.ts`), then runs two phases in order:
   `initializeCache()` (which awaits `evaluationInteractor().initialize()`), and only
   after it resolves, `scheduleAndFetch()` (`scheduleTasks()` → `new
   TaskScheduler().start()`, then the first `fetchEvaluations()`). The ordering is
   load-bearing: `StreamingTask.start()` synchronously reads the evaluation cache via
   `getCurrentEvaluationsCondition()`, which throws if the cache isn't loaded yet.
   The two phases fail differently — a rejected `initializeCache()` is fatal and
   clears the singleton (so a retry re-initializes instead of no-oping on a dead
   instance), while a first-fetch timeout in `scheduleAndFetch()` is normal and must
   **not** tear the singleton down. Both paths identity-check `getInstance() ===
   client` first, so a `destroyBKTClient()` racing initialization can neither clear a
   newer client nor leave tasks scheduled on a discarded one. `resetTasks()` stops the
   scheduler; `destroyBKTClient()` calls it and clears the instance + page lifecycle
   listeners. Browser build also wires `setupPageLifecycleListeners` for
   flush-on-pagehide when `enableAutoPageLifecycleFlush`, guarded by the same identity
   check so a destroyed-mid-init client doesn't leak listeners.
